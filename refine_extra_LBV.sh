...
do
  ...
  cp $param_file $tdir/CBD_phenix.params
...
do
 ...
 do
  ...
  if [ ! -f "${prefix}_001.pdb" ]; 
   ...
   ${seb_phenix}phenix.refine ${tp}_extra_${AF}.mtz monomers.file_name=$cif xray_data.labels="F_${tp},SIGF_${tp}" xray_data.r_free_flags.file_name="${tp}_extra_${AF}.mtz" output.prefix=$prefix CBD_phenix.params --overwrite
   else
